...
- ✨ **柔軟な表示オプション**:
    - **背景色変更**: 透過画像の背景色を（白/黒/透過/赤/青など）自由に切り替え
    - **プレビュー量**: テキストプレビューを「一部（20行）」または「全文」から選択
    - **クリック動作**: リンククリック時に「ファイルを開く」か「フォルダの場所を開く」かを選択可能
- 🔣 **文字コード自動判別**: Shift-JIS等のファイルも文字化けせずに表示
- 🏷️ **自動ラベル**: ファイル名からラベルを自動入力
...
2. **表示オプション**（サイドバー）:
    - **クリック時の動作**:
        - `📄 開く`: ファイルを関連付けられたアプリで開きます
        - `📂 場所`: ファイルが含まれるフォルダをエクスプローラーで開き、そのファイルを選択状態にします
    - **プレビュー量**:
        - `📑 一部`: ファイルの先頭20行のみを表示します（動作が軽量）
        - `📜 全文`: ファイル全体を表示します
3. **背景色変更**（ツールバー）:
    - 画面上部のカラーパレットから、エリア以外の背景色を変更できます（透過部分の視認性向上に利用可能）
4. エリアにマウスをホバーするとプレビューが表示されます
    - **操作**: `Ctrl + V` を押すとプレビューが固定され、マウスで操作（スクロールやコピー）できます。もう一度押すと解除されます。
5. エリアをクリックすると、設定した「クリック時の動作」に従ってファイルやフォルダが開きます。
